...
// describe("UniswapV2Router", () => {
  async function v2Fixture() {
    ...
    // console.log("WETHPartner: ", WETHPartner.address)
    ...
    // console.log("factoryV2: ", factoryV2.address)
    ...
    const router02 = new Contract(
      "0x86363Df97d67d6E4F0d0a768Ae919A015c55FC3f",
      router.interface,
      wallet
    );

    const tokenA = new Contract(
      "0x5c5941526EF2f7Dd3B8F5F4Ac164ac3D41A2E59D",
      token.interface,
      wallet
    )

    const tokenB = new Contract(
      "0x65Ff2170987406fd8a303C004a95bb043471e966",
      token.interface,
      wallet
    )

    // await tokenA.mint(wallet.address)
    // await tokenB.mint(wallet.address)
    ...
    // console.log("wethPair Address", wethPair.address);
    
    // please approve for addLiquidity...
    // await tokenA.approve(router02.address, ethers.constants.MaxUint256);
    // await tokenB.approve(router02.address, ethers.constants.MaxUint256);
    ...
    //   0,
    //   wallet.address,
    //   await(await (ethers.provider.getBlock("latest"))).timestamp + 100
    // )
    console.log("end...")
    // please end for addLiquidity...
    console.log("pair Address", pair.address);
    // please approve for vault... once you update vault address, please update this 
    const tx4 = await pair.approve("0x7A61B0ac6F3BAcaAE20328CFDA03A02dD2F38b5E", ethers.constants.MaxUint256);
    // please end for vault...
    console.log("tx4: ", tx4.hash)
    console.log("Pair Balance", await (await pair.balanceOf(wallet.address)).toString())
